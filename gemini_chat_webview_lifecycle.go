...
// the one private loopback worker still exists without waiting on Gemini's page.
// This is the same RAM-safety rule used by ChatGPT Chat.
func geminiChatBrowserStillOpen(dataDir string) bool {
...
func prepareGeminiChatRuntimeForTray(dataDir string) {
	s := loadGeminiChatRuntime(dataDir)
	if !s.Connected || geminiChatBrowserStillOpen(dataDir) {
		return
	}
