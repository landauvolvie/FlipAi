...
// the one private loopback worker still exists without waiting on Claude's page.
// This is the same RAM-safety rule used by ChatGPT Chat.
func claudeChatBrowserStillOpen(dataDir string) bool {
...
func prepareClaudeChatRuntimeForTray(dataDir string) {
	s := loadClaudeChatRuntime(dataDir)
	if !s.Connected || claudeChatBrowserStillOpen(dataDir) {
		return
	}
