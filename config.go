...
type State struct {
	...
	// LastAgentBySender remembers the most recently selected SMS route for each
	// allowed phone. New values use `route:<ID>` (for example route:OW) so
	// unprefixed follow-ups keep the exact browser experience, not merely the
	// shared execution engine. Legacy single-letter values remain readable.
...
func appPaths() (dataDir, configFile, stateFile, tokenFile string, err error) {
	...
	dataDir = filepath.Join(base, "FlipAi")
