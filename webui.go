...
func (a *App) saveSetup(w http.ResponseWriter, r *http.Request) {
	...
	if v := strings.TrimSpace(r.FormValue("maxReplyParts")); v != "" {
		n, err := strconv.Atoi(v)
		if err != nil || n < 1 || n > 10 {
			renderResult(w, 400, false, "Reply part count is invalid", "Choose between 1 and 10 texts per answer.")
			return
		}
		cfg.GoogleVoice.MaxReplyParts = n
	}
	if v := strings.TrimSpace(r.FormValue("progressInterval")); v != "" {
		n, err := strconv.Atoi(v)
		if err != nil || n < 30 || n > 3600 {
			renderResult(w, 400, false, "Progress interval is invalid", "Choose a progress update interval between 30 and 3600 seconds.")
			return
		}
