...
func runClaudeChatWebView(dataDir string, visible bool) error {
	...
	if w == nil {
		return errors.New("Microsoft Edge WebView2 Runtime could not create the Claude Chat browser")
	}
	defer w.Destroy()
	...
	_ = w.Bind("flipClaudeChatStatus", func(signedIn bool, href string) {
		...
		if signedIn && !wasSignedIn {
			if hadConnected {
				claudeChatActivity(dataDir, "info", "claude-chat-session", "Saved Claude Chat sign-in was restored.", 0)
			} else {
				claudeChatActivity(dataDir, "info", "claude-chat-session", "Claude Chat sign-in was verified and saved in FlipAi's dedicated profile.", 0)
				hadConnected = true
			}
		}
...
func startClaudeChatControlEndpoint(dataDir string, w webview2.WebView, dev voiceDevTools) (int, io.Closer) {
	...
	turn := func(rw http.ResponseWriter, r *http.Request, prompt string, newChat bool) {
		...
		if newChat {
			var ignored bool
			if err := claudeChatEval(dev, `(()=>{location.href='https://claude.ai/new';return true})()`, false, &ignored); err != nil {
		...
		if !waitForClaudeChatPageSignedIn(dev, 25*time.Second) {
			...
			_ = json.NewEncoder(rw).Encode(map[string]any{"ok": false, "detail": "Claude Chat is not signed in inside FlipAi. Press Connect and complete sign-in first."})
			return
		}
...
func recordClaudeChatWorkerError(dataDir string, err error) {
	...
	claudeChatActivity(dataDir, "error", "claude-chat-session", "Claude Chat browser stopped with an error: "+err.Error(), 0)
}
