...
func TestImageFromCodexThreadReadUsesNewestImageGenerationItem(t *testing.T) {
	...
	// Keep it a valid PNG while making the payload distinguishable for the
	// parser test by using the normal test image as the newest item too. The
	// ordering assertion is carried by the invalid older image below.
	oldEncoded := base64.StdEncoding.EncodeToString([]byte("not-an-image"))
	newEncoded := base64.StdEncoding.EncodeToString(newImage)
	...
	if !bytes.Equal(img.Data, newImage) {
