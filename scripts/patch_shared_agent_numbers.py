...
Path("shared_agent_number_test.go").write_text(
r'''package main
...
replace_once("config.go", 'const version = "0.46.1"', 'const version = "0.46.2"')
Path("VERSION").write_text("0.46.2\n", encoding="utf-8")
...
# Remove one-shot patch machinery from the finished branch.
Path(".github/workflows/patch-shared-agent-numbers.yml").unlink(missing_ok=True)
Path("scripts/patch_shared_agent_numbers.py").unlink(missing_ok=True)
