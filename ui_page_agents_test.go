...
func TestAgentsPageOwnsUnifiedAgentSettings(t *testing.T) {
	...
	for _, want := range []string{
		...
		`name="codexPrefix"`, `name="claudePrefix"`, `name="chatgptPrefix"`,
		`name="codexPath"`, `name="claudePath"`, `name="permissionMode"`,
		`name="sharedReplyStyle"`, "Allowed phone numbers", "Security code",
		`name="codexRequireCode"`, `name="claudeRequireCode"`, `name="chatgptRequireCode"`,
		`name="codexAckDelay"`, `name="claudeAckDelay"`, `name="chatgptAckDelay"`,
	...
	if strings.Count(body, `name="sharedReplyStyle"`) != 3 {
		t.Fatalf("shared SMS instruction should be available from all three panes")
	}

	agentFields := []string{`name="codexPath"`, `name="claudePath"`, `name="permissionMode"`, `name="codexPrefix"`, `name="claudePrefix"`, `name="chatgptPrefix"`, `name="sharedReplyStyle"`}
...
// FlipAi has one SMS instruction for all three agents. Editing it in any
...
func TestSharedSMSInstructionIsEditableAndUsedByEveryAgent(t *testing.T) {
	...
	if cfg.Codex.Instruction != "" || cfg.Claude.Instruction != "" || cfg.ChatGPT.Instruction != "" {
