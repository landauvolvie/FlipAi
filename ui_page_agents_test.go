...
func TestAgentsPageOwnsUnifiedAgentSettings(t *testing.T) {
	...
	for _, want := range []string{
		...
		`name="codexPrefix"`, `name="claudePrefix"`, `name="chatgptPrefix"`, `name="claudeChatPrefix"`, `name="geminiChatPrefix"`, `name="grokChatPrefix"`, `name="copilotChatPrefix"`, `name="museChatPrefix"`,
		`name="codexPath"`, `name="claudePath"`, `name="permissionMode"`,
		`name="sharedReplyStyle"`, "Allowed phone numbers", "Security code",
		`name="codexRequireCode"`, `name="claudeRequireCode"`, `name="chatgptRequireCode"`, `name="claudeChatRequireCode"`, `name="geminiChatRequireCode"`, `name="grokChatRequireCode"`, `name="copilotChatRequireCode"`, `name="museChatRequireCode"`,
		`name="codexAckDelay"`, `name="claudeAckDelay"`, `name="chatgptAckDelay"`, `name="claudeChatAckDelay"`, `name="geminiChatAckDelay"`, `name="grokChatAckDelay"`, `name="copilotChatAckDelay"`, `name="museChatAckDelay"`,
	...
	if strings.Count(body, `name="sharedReplyStyle"`) != 8 {
		t.Fatalf("shared SMS instruction should be available from all eight panes")
	}

	agentFields := []string{`name="codexPath"`, `name="claudePath"`, `name="permissionMode"`, `name="codexPrefix"`, `name="claudePrefix"`, `name="chatgptPrefix"`, `name="claudeChatPrefix"`, `name="geminiChatPrefix"`, `name="grokChatPrefix"`, `name="copilotChatPrefix"`, `name="museChatPrefix"`, `name="sharedReplyStyle"`}
...
// FlipAi has one SMS instruction for all eight agents. Editing it in any
...
func TestSharedSMSInstructionIsEditableAndUsedByEveryAgent(t *testing.T) {
	...
	if cfg.Codex.Instruction != "" || cfg.Claude.Instruction != "" || cfg.ChatGPT.Instruction != "" || cfg.ClaudeChat.Instruction != "" || cfg.GeminiChat.Instruction != "" || cfg.GrokChat.Instruction != "" || cfg.CopilotChat.Instruction != "" || cfg.MuseChat.Instruction != "" {
