...
func TestSecurityCodeCanBeDisabledWithoutWeakeningRouting(t *testing.T) {
	...
	for _, tc := range tests {
		...
		rc, err := parseRemoteCommand(tc.raw, cfg, from)
		if err != nil {
...
func TestAgentSecurityCodeIsOffOnAFreshInstall(t *testing.T) {
	// A code is now each agent's own choice, and a new install asks for none.
	cfg := defaultConfig(t.TempDir())
	migrateAgentSettings(&cfg)
	for _, agent := range []string{"C", "A"} {
		if agentSettings(cfg, agent).RequireCode {
			t.Fatalf("%s requires a code on a fresh install", agentDisplayName(agent))
		}
	}
	if _, err := parseRemoteCommand("hello", cfg, "C"); err != nil {
		t.Fatalf("a plain message was refused on a fresh install: %v", err)
	}
}

func TestAgentSecurityCodeIsEnforcedPerAgent(t *testing.T) {
	cfg := defaultConfig(t.TempDir())
	if err := setAgentCode(&cfg, "C", "482913"); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := parseRemoteCommand("hello", cfg, "C"); err == nil {
		t.Fatal("a message without the required code was accepted")
	}
	rc, err := parseRemoteCommand("482913 hello", cfg, "C")
	if err != nil || rc.Agent != "C" || rc.Text != "hello" {
		t.Fatalf("coded route failed: %#v %v", rc, err)
	}
	// Claude never had a code set, so its own numbers are unaffected.
	if _, err := parseRemoteCommand("hello", cfg, "A"); err != nil {
		t.Fatalf("one agent's code was applied to the other: %v", err)
	}
}
...
func TestCodexFullWindowsUserAccessProtocolShapes(t *testing.T) {
	...
	if smoke["ephemeral"] != true {
		t.Fatal("explicit ephemeral smoke-test setting was overwritten")
	}
}
...
func TestCodexFullAccessAndThreadHandoffEndToEnd(t *testing.T) {
	...
	if json.Unmarshal(raw, &started) != nil || started.Thread.ID == "" {
		t.Fatal("missing test thread id")
	}
	...
	for time.Now().Before(deadline) {
		...
		if completed && !c.threadSubscribed(started.Thread.ID) {
			return
		}
