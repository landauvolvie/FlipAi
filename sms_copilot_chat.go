...
func authorizeCopilotChatRaw(raw string, cfg Config) (string, error) {
	...
	if len(f) < 2 {
		return "", errors.New("missing the Microsoft Copilot Chat security code or the command")
	}
	if !verifyAgentCode(s, f[0]) {
		return "", errors.New("invalid SMS security code for Microsoft Copilot Chat")
	}
...
func parseCopilotChatSMSCommand(raw string, cfg Config) (remoteCommand, error) {
	rest, err := authorizeCopilotChatRaw(raw, cfg)
	if err != nil {
	...
	if strings.EqualFold(rest, newWord) || isAgentNewSession(rest, configuredCopilotChatPrefix(cfg), newWord) {
		return remoteCommand{Agent: "P", New: true}, nil
	}
	text := rest
	if tail, ok := stripAgentCommandPrefix(rest, configuredCopilotChatPrefix(cfg)); ok {
		text = tail
	}
	text = strings.TrimSpace(text)
	if text == "" {
		return remoteCommand{}, errors.New("empty Microsoft Copilot Chat command")
	}
	return remoteCommand{Agent: "P", Text: text}, nil
}

type copilotChatSMSReply struct {
	OK             bool   `json:"ok"`
...
func copilotChatBrowserSendWithProgress(ctx context.Context, dataDir, prompt string, onProgress func(string)) (string, error) {
	...
	b, code, err := copilotChatControlRequest(turnCtx, s, http.MethodPost, "/chat", strings.NewReader(string(payload)))
	cancel()
	if err != nil {
		return "", err
	}
	var out copilotChatSMSReply
...
func copilotChatBrowserNewConversation(ctx context.Context, dataDir string) error {
	...
	b, code, err := copilotChatControlRequest(reqCtx, s, http.MethodPost, "/new", strings.NewReader(`{}`))
	cancel()
	if err != nil {
		return err
	}
	if code != http.StatusOK {
		...
		return fmt.Errorf("Microsoft Copilot Chat new-chat request returned HTTP %d", code)
	}
	return nil
}

func (b *Bridge) composeCopilotChatSMSPrompt(command string) string {
	command = strings.TrimSpace(command)
	hint := strings.TrimSpace(b.cfg.replyStyleHintFor("P"))
	if hint == "" {
...
func (b *Bridge) newCopilotChatConversation(ctx context.Context) error {
	return copilotChatBrowserNewConversation(ctx, filepath.Dir(b.statePath))
}
